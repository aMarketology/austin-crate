import Navigation from '@/app/components/Navigation'
import Footer from '@/app/components/Footer'
import Breadcrumbs from '@/app/components/Breadcrumbs'
import ServicesGrid from '@/app/components/ServicesGrid'
import Image from 'next/image'
import Link from 'next/link'
import { Metadata } from 'next'
import { generateServiceMetadata, generateServiceSchema, SITE_CONFIG } from '@/lib/seo-utils'
import SEOWrapper from '@/app/components/SEOWrapper'

const SLUG = '/services/shipping-and-crating/machinery-manufacturing-equipment'

export function generateMetadata(): Metadata {
  return generateServiceMetadata({
    serviceName: 'Machinery & Manufacturing Equipment Shipping Austin TX',
    description:
      'Heavy equipment crating and freight for CNC machines, laser cutters, industrial presses, robotic cells, and manufacturing equipment in Austin, TX. Rigging, skid prep, and LTL coordination. Call (512) 240-9818.',
    keywords: [
      'machinery shipping Austin TX',
      'CNC machine shipping Austin',
      'manufacturing equipment shipping Austin TX',
      'laser cutter shipping Austin',
      'industrial equipment crating Austin TX',
      'heavy machinery freight Austin',
    ],
    serviceUrl: SLUG,
  })
}

export default function MachineryManufacturingEquipment() {
  const serviceSchema = generateServiceSchema({
    serviceName: 'Machinery & Manufacturing Equipment Shipping Austin TX',
    description:
      'Heavy rigging, skid preparation, and freight coordination for CNC machines, laser cutters, industrial presses, and manufacturing equipment in Austin, TX.',
    serviceType: 'Industrial Equipment Shipping',
  })

  return (
    <SEOWrapper slug={SLUG}>
      <div className="min-h-screen bg-white flex flex-col">
        <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(serviceSchema) }} />
        <Navigation />
        <Breadcrumbs />

        {/* Hero */}
        <section className="bg-gradient-to-br from-grey-900 to-grey-800 text-white pt-28 md:pt-32 pb-16 px-4">
          <div className="max-w-5xl mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div>
            <Link href="/services/shipping-and-crating" className="text-emerald hover:underline mb-4 inline-block text-sm">
              ← Shipping & Crating Services
            </Link>
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-grey-700 bg-grey-800/50 mb-6 ml-4">
              <span className="text-xs font-medium text-grey-300 uppercase tracking-widest">Industrial Shipping · Austin, TX</span>
            </div>
            <h1 className="text-4xl md:text-5xl font-bold mb-5">Machinery & Manufacturing Equipment Shipping in Austin, TX</h1>
            <p className="text-xl text-grey-300 mb-8 max-w-3xl">
              CNC machining centers, laser cutters, and industrial presses don&apos;t ship in a box. We rig it, skid it, coordinate the freight, and deliver it ready to be set in place, no carrier surprises, no equipment damage.
            </p>
            <div className="flex flex-col sm:flex-row gap-4">
              <a
                href={`tel:${SITE_CONFIG.phone}`}
                className="inline-flex items-center justify-center gap-3 px-8 py-4 bg-emerald rounded-lg font-medium text-white hover:bg-emerald/90 transition-all duration-300"
              >
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                  <path d="M2 3a1 1 0 011-1h2.153a1 1 0 01.986.836l.74 4.435a1 1 0 01-.54 1.06l-1.548.773a11.037 11.037 0 006.105 6.105l.774-1.548a1 1 0 011.059-.54l4.435.74a1 1 0 01.836.986V17a1 1 0 01-1 1h-2C7.82 18 2 12.18 2 5V3z" />
                </svg>
                {SITE_CONFIG.phone}
              </a>
              <Link
                href="/contact"
                className="inline-flex items-center justify-center gap-3 px-8 py-4 bg-grey-800/50 border border-grey-700 rounded-lg font-medium text-grey-200 hover:bg-grey-700/50 backdrop-blur-sm transition-all duration-300"
              >
                Request a Quote
              </Link>
            </div>
          </div>
            <div className="relative h-96 rounded-2xl overflow-hidden border-2 border-grey-700 shadow-xl">
              <Image
                src="/1.jpg"
                alt="Machinery and manufacturing equipment shipping Austin Texas"
                fill
                className="object-cover"
                priority
                fetchPriority="high"
                sizes="(max-width: 1024px) 100vw, 50vw"
                quality={80}
              />
            </div>
          </div>
</div>
        </section>

        {/* Content */}
        <section className="py-14 px-4 bg-white">
          <div className="max-w-4xl mx-auto grid md:grid-cols-2 gap-10">
            <div>
              <h2 className="text-2xl font-bold text-gray-900 mb-4">What We Ship</h2>
              <ul className="space-y-2 text-gray-700">
                {[
                  'CNC machining centers (3-, 4-, 5-axis)',
                  'Laser cutters and plasma cutters',
                  'Industrial presses and stamping equipment',
                  'Lathes, mills, and grinders',
                  'Injection molding machines',
                  'Robotic arms and robotic cells',
                  'Industrial conveyor systems',
                ].map((item) => (
                  <li key={item} className="flex items-start gap-2">
                    <span className="text-emerald mt-1 shrink-0">✓</span>
                    {item}
                  </li>
                ))}
              </ul>
            </div>
            <div>
              <h2 className="text-2xl font-bold text-gray-900 mb-4">How We Handle It</h2>
              <ul className="space-y-2 text-gray-700">
                {[
                  'Heavy rigging and machinery-ready crews',
                  'Skid construction and bolt-down preparation',
                  'Liftgate and flatbed freight coordination',
                  'Fluid system drain documentation',
                  'Disassembly and reassembly planning',
                ].map((item) => (
                  <li key={item} className="flex items-start gap-2">
                    <span className="text-emerald mt-1 shrink-0">✓</span>
                    {item}
                  </li>
                ))}
              </ul>
            </div>
          </div>
          <div className="max-w-4xl mx-auto mt-10 p-6 bg-gray-50 rounded-xl border border-gray-200">
            <p className="text-gray-700">
              We coordinate LTL and flatbed freight for equipment that exceeds standard carrier weight limits, and we build the skids on-site. If your machine needs to move across Austin or across the country, call us for a straight quote.
            </p>
          </div>
        </section>

        <ServicesGrid title="More Specialty Shipping Services" showSubLinks exclude={['/services/shipping-and-crating']} />

        <Footer />
      </div>
    </SEOWrapper>
  )
}
